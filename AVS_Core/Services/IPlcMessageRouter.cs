        ...
        public async Task HandleMessageAsync(string connectionPlcId, string rawMessage)
        {
            try
            {
                ...
                if (funcCode == "2003") // 检测
                {
                    ...
                    {
                        case "0001":
                            response = await HandleInspectInit(connectionPlcId, sessionConfig);
                            break;
                        case "0002":
                            response = await HandleInspectResult(connectionPlcId, sessionConfig);
                            break;
                        default:
                            response = CreateErrorResponse(sessionConfig, "Unknown step");
                            break;
                    }
                }
                else if (funcCode == "2001") // 标定\点检
                {
                    ...
                    {
                        case "0001":
                            response = await HandleCalibInit(connectionPlcId, sessionConfig, isverify);
                            break;
                        case "0002":
                            response = HandleCalibResult(connectionPlcId, sessionConfig);
                            break;
                        default:
                            response = CreateErrorResponse(sessionConfig, "Unknown step");
                            break;
