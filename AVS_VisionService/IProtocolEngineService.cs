...
namespace AVS_Service
{
    public interface IProtocolEngineService
    {
        ...
        /// 用于同步 UI 中的修改到底层
        /// </summary>
        void UpdateOutputFields(List<ProtocolField> outputFields);
        /// <summary>
        /// 更新输入字段配置
        /// </summary>
    ...
    public class ProtocolEngineService : IProtocolEngineService
    {
        ...
        //保存当前配置的引用
        private List<ProtocolField> _currentOutputFields = new List<ProtocolField>();
        private List<ProtocolField> _currentInputFields = new List<ProtocolField>();
        public List<ProtocolField> GetOutputFields() => _currentOutputFields;
        public List<ProtocolField> GetInputFields() => _currentInputFields;
        ...
        private string GetFieldContent(ProtocolField field)
        {
            switch (field.Source)
            {
                ...
                case FieldSource.Variable:// 从变量池取值
                    ...
                    //使用默认值
                    return field.FixedValue ?? "";
                case FieldSource.Padding:  // 填充模式返回空,长度补齐
                    return "";
        ...
        public void UpdateOutputFields(List<ProtocolField> outputFields)
        {
            if (outputFields == null)
            {
                _logger?.Warning("输出字段配置为 null");
                return;
            }
            _currentOutputFields = outputFields.ToList();
            _logger?.Information("输出字段配置已更新，共 {Count} 个字段", outputFields.Count);
        }

        public void UpdateInputFields(List<ProtocolField> inputFields)
        {
            if (inputFields == null)
            {
                _logger?.Warning("输入字段配置为 null");
                return;
            }
            _currentInputFields = inputFields.ToList();
            _logger?.Information("输入字段配置已更新，共 {Count} 个字段", inputFields.Count);
        }
    }
}
